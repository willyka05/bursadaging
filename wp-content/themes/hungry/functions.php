...
function hungry_setup() {
	...
	add_image_size( 'hungry-large-thumb', 830 );
	add_image_size( 'hungry-medium-thumb', 550, 400, true );
	add_image_size( 'hungry-small-thumb', 230 );
	
	//Register nav menu used in theme.
	register_nav_menus( array(
		'primary' 	=> __( 'Primary Menu', 'hungry' )
	) );
...
add_action('hungry_before_site', 'hungry_preloader');
...
function hungry_grid_post_meta(){
...
function hungry_excerpt_more($more) {
	global $post;
	$layout = theme_blog_layout();
	return ' ....&nbsp;&nbsp;<a class=" '.$layout.'" href="'. get_permalink($post->ID) . '">' . __('Read More', 'hungry') . '</a>';
}
...
function hungry_scripts() {
	...
	wp_enqueue_script( 'hungry-common', get_template_directory_uri() . '/js/common.js', array('jquery'), '20180213', true );
	...
	wp_enqueue_style( 'hungry-font-awesome', get_template_directory_uri() . '/fonts/font-awesome.min.css' );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
	
	/* wp_enqueue_style( 'hungry-bootstrap', get_template_directory_uri() . '/css/bootstrap/bootstrap.min.css', array(), true ); */
 ...
 * Enqueue Bootstrap
 */
function hungry_enqueue_bootstrap() {
	wp_enqueue_style( 'hungry-bootstrap', get_template_directory_uri() . '/css/bootstrap/bootstrap.min.css', array(), true );
}
add_action( 'wp_enqueue_scripts', 'hungry_enqueue_bootstrap', 9 );
...
function hungry_widgets_init() {
	...
	for ($i=1; $i<=3; $i++){
		register_sidebar( array(
			'name' => __( 'Footer-widget-area-', 'hungry' ) . $i,
...
function hungry_edit_link() {
	edit_post_link(
		sprintf( __( '<i class="fa fa-edit"></i>Edit<span class="screen-reader-text"> "%s"</span>', 'hungry' ), get_the_title() ),
...
function hungry_get_breadcrumb() {
    echo '<a href="'.home_url().'" rel="nofollow">'.__('Home', 'hungry').'</a>';
    if ( is_category() || is_single() ) {
        ...
        echo __("&nbsp;&nbsp;&#47;&nbsp;&nbsp;Search Results for... ", "hungry");
        echo '"<em>';
...
function hungry_recommended_plugin() {
...
add_action( 'tgmpa_register', 'hungry_recommended_plugin' );
...
add_filter( 'pre_set_site_transient_update_themes', 'hungry_check_for_update' );
function hungry_check_for_update( $checked_data ){
...
popularfx_maybe_promo([
	'after' => 1,// In days
	'interval' => 30,// In days
	'pro_url' => 'https://popularfx.com/themes/wordpress/corporate/Hungry_Pro',
	'rating' => 'https://popularfx.com/themes/wordpress/corporate/Hungry',
	'twitter' => 'https://twitter.com',
	'facebook' => 'https://www.facebook.com/',
	'website' => 'http://suavethemes.com',
]);

function hungry_upsell_script() {
...
add_action( 'customize_controls_enqueue_scripts', 'hungry_upsell_script' );